...
start = st.button("🎥 Start Live Webcam Detection")
st.markdown('</div>', unsafe_allow_html=True)

if start:
    st.success("Webcam started (connect backend here)")
...
with left:
    ...
    analyze = st.button("🔍 Upload & Analyze")
    st.markdown('</div>', unsafe_allow_html=True)

    if analyze:
        st.info("Analyzing video... (connect AI model here)")
...
with right:
    ...
    for key, value in stats.items():
        st.markdown(
            f'<div class="stat-box">{key}<br><b>{value}</b></div>',
            unsafe_allow_html=True
        )
    ...
    alerts = [
        ("Accident", "2026-03-16 08:40"),
        ("Accident", "2026-03-16 08:39"),
        ("Accident", "2026-03-16 08:38"),
    ]

    for title, time in alerts:
        st.markdown(
            f'<div class="alert">📍 {title}<br><small>{time}</small></div>',
            unsafe_allow_html=True
        )
